...
def is_content_health():
    ...
    count = ContentAppStatus.objects.filter(name__endswith=socket.gethostname(), last_heartbeat__gte=age_threshold).count()
...
def is_worker_health():
    ...
    age_threshold = timezone.now() - timedelta(seconds=30)
    count = Worker.objects.filter(name__endswith=socket.getfqdn(), last_heartbeat__gte=age_threshold).count()
