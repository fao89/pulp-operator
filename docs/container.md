...
- **pulpcore-resource-manager** - a different type of pulp-worker that plays a coordinating role for the tasking system. All asynchronous work flows through this service. Only a single entity does work, but other instances can be run as hot spares that will take over if the active one fails.
...
* `latest`: Nightly built, with master/main branches of each plugin.
...
An Nginx imaged based on [centos/nginx-116-centos7](https://hub.docker.com/r/centos/nginx-116-centos7),
with pulpcore and plugins specific configuration.

### Tags

* `latest`: Nightly built, with master/main branches of [pulpcore](https://github.com/pulp/pulpcore) and its plugins.
* `stable`: Built on push, with latest released version of each plugin.
* `3.y.z`:  Pulpcore 3.y.z version and its compatible plugins.
...
* `latest`: Nightly built, with master branch of [galaxy](https://github.com/ansible/galaxy_ng).
...
* `latest`: Nightly built, with master branch of galaxy.
* `stable`: Built on push, with latest released version of galaxy.
* `4.y.z`:  Galaxy 4.y.z version.
...
* `latest`: Nightly built, with main branch of [pulp-operator](https://github.com/pulp/pulp-operator).
