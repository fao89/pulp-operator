...
if [[ "$1" == "--kind" ]] || [[ "$1" == "-k" ]]; then
  ...
  kind export kubeconfig --name osdk-test
   ...
   kubectl port-forward service/example-pulp-web-svc 24880:24880 &
fi
...
echo ::group::OBJECTS
 kubectl get pulp,pulpbackup,pulprestore,pvc,configmap,serviceaccount,secret,networkpolicy,ingress,service,deployment,statefulset,hpa,job,cronjob -o yaml
echo ::endgroup::
